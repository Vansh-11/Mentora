...
const CYBER_SECURITY_AGENT_ID = "306705d5-2115-40d5-b664-b5f70ea39533";
const DIALOGFLOW_SCRIPT_URL = "https://www.gstatic.com/dialogflow-console/fast/messenger/bootstrap.js?v=1";
const DF_SCRIPT_ID = "dialogflow-bootstrap-script-cyber"; 

export default function CyberSecurityHelpPage() {
  ...
  useEffect(() => {
    ...
    const messengerSelector = `df-messenger[agent-id="${CYBER_SECURITY_AGENT_ID}"]`;
    
    const handleMessengerLoaded = () => {
      console.log(`CyberSecurityHelpPage: df-messenger-loaded event received for agent ${CYBER_SECURITY_AGENT_ID}.`);
    ...
    const loadAndInitializeMessenger = () => {
      if (document.querySelector(messengerSelector)) {
        console.log("CyberSecurityHelpPage: Dialogflow Messenger for this agent already in DOM.");
      ...
      if ((window as any).dfMessengerBootstrapLoaded) {
        console.log("CyberSecurityHelpPage: Dialogflow bootstrap script already loaded globally.");
        setRenderMessenger(true);
        return;
      }
      ...
      if (!script) {
        ...
        script.onload = () => {
          console.log("CyberSecurityHelpPage: Dialogflow bootstrap.js loaded.");
          (window as any).dfMessengerBootstrapLoaded = true;
          setRenderMessenger(true);
        };
        script.onerror = () => {
          console.error("CyberSecurityHelpPage: Failed to load Dialogflow bootstrap.js.");
        };
      } else {
        const handleExistingScriptLoad = () => {
          if (!(window as any).dfMessengerBootstrapLoaded) {
            console.log("CyberSecurityHelpPage: Dialogflow bootstrap.js (existing tag) loaded.");
            (window as any).dfMessengerBootstrapLoaded = true;
          }
          setRenderMessenger(true);
        };
        if ((window as any).dfMessengerBootstrapLoaded) {
            ...
            script.addEventListener('error', () => console.error("CyberSecurityHelpPage: Failed to load Dialogflow bootstrap.js (existing tag)."));
        }
      }
    };
    ...
    return () => {
      ...
      if (existingMessenger) {
        console.log("CyberSecurityHelpPage: Removing Dialogflow Messenger for this agent on unmount.");
  ...
  useEffect(() => {
    ...
    const dfMessengerElement = document.querySelector(`df-messenger[agent-id="${CYBER_SECURITY_AGENT_ID}"]`);
    const handleDfMessengerLoadedEvent = () => {
      console.log(`CyberSecurityHelpPage: df-messenger-loaded event received for agent ${CYBER_SECURITY_AGENT_ID} (useEffect).`);
      setIsMessengerComponentReady(true);
    };
  ...
  const isChatVisiblyOpen = (): boolean => {
    ...
    const dfMessenger = document.querySelector(`df-messenger[agent-id="${CYBER_SECURITY_AGENT_ID}"]`) as HTMLElement | null;
  ...
  const openChatWidgetIfNotOpen = () => {
     if (!isClient) {
      console.warn('CyberSecurityHelpPage: Attempted to open chat before client hydration.');
      return;
    }
    
    const dfMessenger = document.querySelector(`df-messenger[agent-id="${CYBER_SECURITY_AGENT_ID}"]`) as any;

    if (!dfMessenger) {
      console.warn(`CyberSecurityHelpPage: df-messenger with agent-id ${CYBER_SECURITY_AGENT_ID} not found when trying to open.`);
      return;
    }
    
    if (!dfMessenger.shadowRoot) {
        console.warn("CyberSecurityHelpPage: df-messenger shadowRoot NOT FOUND when trying to open.");
    ...
    if (widgetIcon && !isChatVisiblyOpen()) {
      console.log("CyberSecurityHelpPage: #widgetIcon FOUND. Clicking to open.");
      widgetIcon.click();
    } else if (isChatVisiblyOpen()) {
      console.log("CyberSecurityHelpPage: Chat already open.");
    } else {
      console.warn("CyberSecurityHelpPage: #widgetIcon NOT FOUND in shadow DOM when trying to open. isMessengerComponentReady:", isMessengerComponentReady);
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      <Header 
        ...
        subtitle="Your Guide to Staying Safe Online"
        description="Learn how to protect yourself from online threats, scams, and privacy risks. Talk to our assistant for tips and support."
      ...
      <main className="flex-grow container mx-auto py-8 md:py-12">
        <section className="grid md:grid-cols-2 gap-8 items-center mb-12 md:mb-16">
          <div>
            ...
            <p className="text-lg text-foreground/85 mb-6">
             In today's digital world, knowing how to stay safe online is essential. Our Cyber Security assistant is here to help you with:
            </p>
            <ul className="space-y-3 text-foreground/80 list-disc list-inside mb-6">
              <li>Identifying phishing emails and scams.</li>
              <li>Creating strong, secure passwords.</li>
              <li>Understanding privacy settings on social media.</li>
              <li>What to do if you think your account has been compromised.</li>
            </ul>
            <p className="text-lg text-foreground/85 mb-6">
              Your conversation is confidential. Click below to start getting advice on how to protect yourself.
            </p>
            <Button 
              size="lg" 
              className="bg-accent hover:bg-accent/90 text-accent-foreground"
              onClick={openChatWidgetIfNotOpen}
              aria-label="Talk to a support agent about cyber security"
            >
        ...
        <section className="bg-primary/10 p-6 md:p-8 rounded-xl shadow-md text-center">
          ...
          <p className="text-md md:text-lg text-foreground/85 max-w-2xl mx-auto">
            Stay one step ahead of online threats. Use the chat to ask questions and learn best practices for online safety. Your security is important.
          </p>
        </section>
      </main>
      ...
      {isClient && renderMessenger && (
        <df-messenger
          intent="WELCOME"
          chat-title="Cyber Security"
          agent-id={CYBER_SECURITY_AGENT_ID}
          language-code="en"
        >
