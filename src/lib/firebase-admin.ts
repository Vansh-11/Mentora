...
function initializeFirebaseAdmin() {
  ...
  // This is a more reliable way to determine the environment than a generic try-catch.
  if (process.env.GCLOUD_PROJECT) {
    console.log("GCLOUD_PROJECT detected. Initializing with default credentials.");
    try {
      admin.initializeApp();
      console.log("Firebase Admin SDK initialized successfully with default credentials.");
    } catch (error) {
      console.error("CRITICAL: Default credential initialization failed in a GCloud environment.", error);
    }
  } else {
    // Fallback for local development when not in a GCloud environment
    console.log("No GCLOUD_PROJECT. Initializing with hardcoded service account for local dev.");
    try {
